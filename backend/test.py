...
def get_landmark_coordinates(image_path: str, output_csv: str = './output.csv'):
    ...
    # Prepare a list to store the landmark coordinates
    landmarks_data = []

    # Iterate over face landmarks and extract coordinates
    for face_landmarks in results.multi_face_landmarks:
        for idx, landmark in enumerate(face_landmarks.landmark):
            # Convert normalized coordinates to pixel values
            x = int(landmark.x * img_width)
            y = int(landmark.y * img_height)
            landmarks_data.append([idx, x, y, image_name, 542, 739])
            print(f"Landmark {idx}: (X: {x}, Y: {y}, Image: {image_name})")
...
def load_filter(filter_name="dog"):
    ...
    for filter in filters:
        ...
        # Example usage
        get_landmark_coordinates('face.jpg')
